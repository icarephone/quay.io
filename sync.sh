...
git_init(){
    git config --global user.name "sqeven"
    git config --global user.email "admin@sqeven.com"
    git remote rm origin
    git remote add origin git@github.com:icarephone/quay.io.git
    git pull
...
image_tag(){
    docker pull $1:$2
    docker tag $1:$2 registry.cn-shenzhen.aliyuncs.com/$3:$2
    docker rmi $1:$2
}
...
# img_name tag
hub_tag_exist(){
    curl -s https://registry.cn-shenzhen.aliyuncs.com/v2/repositories/${MY_REPO}/$1/tags/$2/ | jq -r .name
}
